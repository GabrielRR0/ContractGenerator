...
- **`useContractWizard.ts`**: unico composable del dominio, orquesta todo el flujo:
  ...
  - `preview`: contenido (`titulo`/`parrafos`/`firmas`) que se re-pide a `POST /api/contracts/preview` cada vez que cambian `templateId`, `data` o `locale` (con un debounce de 200ms para no pegarle al backend en cada tecla).
...
La logica pura de validacion (`validateRequiredFields`) esta en `utils/validators/validateContractFields.ts` para poder reutilizarla o testearla sin depender de la reactividad de Vue. Es generica: recibe los `campos` (`FieldSpec[]`) de la plantilla activa, no una lista de campos fija.
