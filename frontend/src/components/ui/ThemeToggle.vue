...
function alternar(evento: MouseEvent) {
  // Nota: por pedido explicito del usuario, el barrido se muestra siempre,
  // sin chequear prefers-reduced-motion (a diferencia de la practica
  // habitual de accesibilidad).
  const soportaViewTransition = 'startViewTransition' in document

  if (!soportaViewTransition) {
    guardarYAplicar()
    return
  }
...
/* Los 3 elementos (sol, luna, perilla) son cuadrados de 1.375rem posicionados
   ...
   queda matematicamente centrado sobre la perilla, en vez de aproximado via
   flexbox (que dejaba un desfasaje de un par de px). */
.theme-toggle {
  position: relative;
  width: 3.25rem;
  height: 1.75rem;
...
.icono.activo {
  /* El icono activo queda siempre sobre la perilla (mismo color de acento),
     por eso usa el color de contraste de la perilla, no el acento. */
  opacity: 1;
